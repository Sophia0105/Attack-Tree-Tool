...
dropdown = dbc.Row(
    [
        ...
        dbc.Col(dbc.Button("Update Options", id='update_dropdown_edge', n_clicks=0, class_name="button"),width=2)
    ],
    className="mb-3",
)
...
def alter_output(n_clicks, edge_w, node_nr):
    if n_clicks > 0:
        outp = data_input.alter_edge(node_nr, edge_w)
        return outp
    else: 
        pass
...
layout = dbc.Container([html.H1("Alter the edge weight", className='app-header'), form, update_button_alter, figure], fluid=True)