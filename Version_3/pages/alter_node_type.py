...
dropdown = dbc.Row(
    [
        ...
        dbc.Col(dbc.Button("Update Options", id='update_dropdown_nnr_options', n_clicks=0, class_name="button"),width=2)
    ],
    className="mb-3",
)
...
dropdown_type = dbc.Row(
    [
        dbc.Label("Node Type", html_for="dropdown_type", width=2),
        dbc.Col(dcc.Dropdown(
            id='dropdown_type',
            options=[{'label': 'LEAF', 'value':'end'},
                     {'label': 'AND', 'value': 'and'},
                     {'label': 'OR', 'value': 'or'}],
            value='end'
        ), width=10),
